...
WITH asset AS (
    ...
    FROM account_asset_asset _ass
        LEFT JOIN (SELECT asset_id, SUM(amount) AS depreciated_value
              ...
              WHERE depreciation_date >= '%(from_date)s' AND depreciation_date < '%(to_date)s'
              GROUP BY asset_id
    ...
    , depreciated_info AS (
        SELECT asset.id,
            ...
            (asset.method_number - COALESCE(COUNT(distinct _aml_db.move_id), 0)) AS depreciated_remain
        FROM asset
            JOIN account_asset_depreciation_line _assl ON asset.id = _assl.asset_id
    ...
    , depreciating_info AS (
        ...
        FROM asset
            ...
            JOIN (SELECT * FROM account_move_line
                  WHERE debit > 0
                        AND date_maturity >= '%(from_date)s' AND date_maturity < '%(to_date)s'
                  )
                _aml_db ON _aml_db.move_id = _assmv.account_move_id
            JOIN account_move _amv ON _aml_db.move_id = _amv.id
            JOIN (SELECT * FROM account_move_line
    ...
    , summary AS (
        ...
        FROM depreciated_info LEFT JOIN depreciating_info ON depreciated_info.id = depreciating_info.id
    )
    ...
    def generate_main_content(self):
        ...
        for line in lines_data:
            ...
            self.sheet.write('K{}'.format(s_row), line.get('sum_debit'),
                             self.format_table)
            self.sheet.write('L{}'.format(s_row), line.get('depreciated_value'),
                             self.format_table)
