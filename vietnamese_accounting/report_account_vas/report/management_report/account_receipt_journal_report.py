...
class account_receipt_journal_xls_parser(report_base_vn.Parser):
    ...
    def __init__(self, cr, uid, name, context):
        super(account_receipt_journal_xls_parser, self).__init__(cr, uid, name, context=context)
    ...
    def get_debit_account_data(self):
        ...
        params  = { 'target_move': '', 'account_id': account_id, 'date_start': date_info['date_from_date'], 'date_end': date_info['date_to_date'] }
...
class account_receipt_journal_xls(report_xls_utils.generic_report_xls_base):
    ...
    def __init__(self, name, table, rml=False, parser=False, header=True, store=False):
        super(account_receipt_journal_xls, self).__init__(name, table, rml, parser, header, store)
    ...
    def generate_xls_header(self, _p, _xs, data, objects, wb, ws, row_pos, report_name):
        ...
        # Title address 1
        c_specs = [
            ('company_name', 6, 0, 'text', u'Đơn vị: %s' % _p.get_company()['name'] or ''  , '', cell_address_style),
        ]
        ...
        # Title 2
        c_specs = [
            ('company_name', 6, 0, 'text', u'Địa chỉ: %s' % _p.get_company()['address'] or '', '', cell_address_style),
            ('empty1', 1, 0, 'text', ''),
        ]
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        row_pos = self.xls_write_row(ws, row_pos, row_data, row_style=self.normal_style)
        ...
        # Title 3
        c_specs = [
            ('company_name', 6, 0, 'text', u'MST: %s' % _p.get_company()['vat'] or '', '', cell_address_style),
            ('empty1', 1, 0, 'text', ''),
        ]
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        row_pos = self.xls_write_row(ws, row_pos, row_data, row_style=self.normal_style)
        ...
        c_specs = [
            ('payment_journal', 6, 0, 'text', report_name, None, cell_title_style)
        ]
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        row_pos = self.xls_write_row(ws, row_pos, row_data, row_style=self.normal_style)
        ...
        c_specs = [
            ('amount_on_account', 6, 0, 'text',u'%s %s' % (title,_p.get_account_info().get('code',''),))
        ]
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        row_pos = self.xls_write_row(ws, row_pos, row_data, row_style=self.normal_style)
        ...
        ws.row(row_pos).height = 300
        c_specs = [
            ('from_to', 6, 0, 'text', u'Tên tài khoản: %s' % (_p.get_account_info().get('name','')), None)
        ]
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        row_pos = self.xls_write_row(ws, row_pos, row_data, row_style=self.normal_style)
        ...
        ws.row(row_pos).height = 300
        c_specs = [
            ('asset_book', 6, 0, 'text','Từ %s đến %s' % (_p.get_date().get('date_from','.......'),_p.get_date().get('date_to','.......')))
        ]
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        ...
        ws.write_merge(row_title_body_pos, row_title_body_pos+1, 4, 4, counterpart_account_title, self.normal_style_bold_borderall )
        ws.write_merge(row_title_body_pos, row_title_body_pos+1, 5, 5, 'Số tiền', self.normal_style_bold_borderall )
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        # set print sheet
        ws = super(account_receipt_journal_xls, self).generate_xls_report(_p, _xs, data, objects, wb, report_name)
        row_pos = 0
...
#         # account move lines
        ...
        if not debit_account_data:
            ...
            self.col_specs_template['G']['totals'] = self.col_specs_template['G']['totals_no_lines']
...
account_receipt_journal_xls('report.account.receipt.journal.report','account.move.line', parser=account_receipt_journal_xls_parser)
