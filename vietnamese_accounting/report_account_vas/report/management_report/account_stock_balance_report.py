...
class account_stock_balance_xls_parser(report_base_vn.Parser):
    ...
    def __init__(self, cr, uid, name, context):
        super(account_stock_balance_xls_parser, self).__init__(cr, uid, name, context=context)
    ...
    def get_lines_data(self):
        ...
        SQL = """
        /*
        When receiving product, each stock_move will have only one Journal Entry
        In 2 Journal Items of this Journal, we have stock_move_id is stock_move this item belong to.
        Depend on stock_move and account_move_line, we will get all info for stock_balance report.
        */
...
class account_stock_balance_xls(report_xls_utils.generic_report_xls_base):
    ...
    def __init__(self, name, table, rml=False, parser=False, header=True, store=False):
        super(account_stock_balance_xls, self).__init__(name, table, rml, parser, header, store)
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        # set print sheet
        ws = super(account_stock_balance_xls, self).generate_xls_report(_p, _xs, data, objects, wb, report_name)
        row_pos = 0

        cell_address_style = self.get_cell_style(['bold', 'wrap', 'left'])
        # Title address 1
        c_specs = [
            ('company_name', 7, 0, 'text', u'Đơn vị: %s' % _p.get_company()['name']  , '', cell_address_style),
        ]
        ...
        # Title 2
        c_specs = [
            ('company_name', 7, 0, 'text', u'Địa chỉ: %s' % _p.get_company()['address'], '', cell_address_style),
        ]
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        row_pos = self.xls_write_row(ws, row_pos, row_data, row_style=self.normal_style)
        ...
        # Title 3
        c_specs = [
            ('company_name', 7, 0, 'text', u'MST: %s' % _p.get_company()['vat'], '', cell_address_style),
        ]
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        row_pos = self.xls_write_row(ws, row_pos, row_data, row_style=self.normal_style)
        ...
        c_specs = [
            ('title', 10, 0, 'text', u'Loại tài khoản: %s' % (_p.get_account_info().get('code','')))
        ]
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        row_pos = self.xls_write_row(ws, row_pos, row_data, row_style=self.normal_style)
        ...
        ws.row(row_pos).height = 300
        c_specs = [
            ('title', 10, 0, 'text', u'Tên tài khoản: %s' % (_p.get_account_info().get('name','')))
        ]
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        row_pos = self.xls_write_row(ws, row_pos, row_data, row_style=self.normal_style)
        ...
        ws.row(row_pos).height = 300
        c_specs = [
            ('title', 10, 0, 'text', u'Tên kho: %s' % (_p.get_wizard_data().get('location_name','')))
        ]
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        row_pos = self.xls_write_row(ws, row_pos, row_data, row_style=self.normal_style)
        ...
        ws.row(row_pos).height = 300
        c_specs = [
            ('from_to', 10, 0, 'text', u'Từ %s đến %s' % (_p.get_date().get('date_from','.......'),_p.get_date().get('date_to','.......')))
        ]
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        ...
        c_specs = [
            ('col1', 1, 12, 'text', 'Mã nguyên liệu, vật liệu, công cụ, dụng cụ (sản phẩm, hàng hóa)', None),
            ('col2', 1, 34, 'text', 'Tên nguyên liệu, vật liệu, công cụ, dụng cụ (sản phẩm, hàng hóa)', None ),
        ...
        # merge cell
        ws.write_merge(row_title_body_pos, row_title_body_pos+1, 0, 0, 'Mã nguyên liệu, vật liệu, công cụ, dụng cụ (sản phẩm, hàng hóa)', self.normal_style_bold_borderall )
        ws.write_merge(row_title_body_pos, row_title_body_pos+1, 1, 1, 'Tên nguyên liệu, vật liệu, công cụ, dụng cu (sản phẩm, hàng hóa)', self.normal_style_bold_borderall )
...
#         # account move lines
        ...
        self.col_specs_template['A']['totals'] = [2, 0, 'text', 'Tổng Cộng', None, self.normal_style_bold_borderall]
        if get_lines_data:
        ...
        c_specs = map(lambda x: self.render(x, self.col_specs_template, 'totals'), sum_columns)
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
...
account_stock_balance_xls('report.stock_balance_report','stock.move', parser=account_stock_balance_xls_parser)
