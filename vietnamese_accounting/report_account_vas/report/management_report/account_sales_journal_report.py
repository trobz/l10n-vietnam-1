...
class account_sales_journal_xls_parser(APJR.account_purchases_journal_xls_parser):

    def __init__(self, cr, uid, name, context):
        super(account_sales_journal_xls_parser, self).__init__(cr, uid, name, context=context)
    ...
    def get_lines_data(self):
        ...
        for account in ACCOUNT_MAPPING:
            for acc in account.get('dr',[]):
                acc_dr_ids += account_obj.search(self.cr, self.uid, [('code', '=like', '%s%%' % acc)])
        # get all account id of list account credit
            for acc in account.get('cr', []):
                acc_cr_ids += account_obj.search(self.cr, self.uid, [('code', '=like', '%s%%' % acc)])

        params  = {
                    'amv_dr_state': self.get_wizard_data()['target_move'] == 'posted' and ''' AND amv_dr.state = 'posted' ''' or '',
                    'amv_cr_state': self.get_wizard_data()['target_move'] == 'posted' and ''' AND amv_cr.state = 'posted' ''' or '',
                    'date_from': date_info['date_from_date'], 'date_to': date_info['date_to_date'],
                    'acc_dr_ids': tuple(acc_dr_ids + [-1,-1]), 'acc_cr_ids': tuple(acc_cr_ids + [-1,-1])
                    }
...
#         """
        ...
        array_to_string(array_agg(distinct amv_line_cr.code),',') AS credit_code
        ...
        LEFT JOIN (
            ...
            WHERE
                ...
                AND amv_line_cr.credit <> 0.0
                            AND amv_line_cr.date <= '%(date_to)s' AND amv_line_cr.date >= '%(date_from)s'
                            %(amv_cr_state)s

            ) amv_line_cr ON amv_line_cr.counter_move_id=amv_line_dr.id

        JOIN account_account dr_account ON amv_line_dr.account_id = dr_account.id
        JOIN account_move amv_dr ON amv_dr.id=amv_line_dr.move_id
        JOIN account_journal acj ON acj.id = amv_dr.journal_id

    WHERE
        amv_line_dr.account_id in %(acc_dr_ids)s
        AND amv_line_dr.debit <> 0.0
                AND amv_line_dr.date <= '%(date_to)s' AND amv_line_dr.date >= '%(date_from)s'
                %(amv_dr_state)s
                AND acj.type = 'sale'
...
APJR.account_purchases_journal_xls('report.sales_journal_report','account.move', parser=account_sales_journal_xls_parser)
