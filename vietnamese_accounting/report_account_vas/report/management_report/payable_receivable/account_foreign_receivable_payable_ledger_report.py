...
from openerp.addons.report_xlsx.report_xlsx import report_xlsx  # @UnresolvedImport
...
class account_foreign_receivable_payable_ledger_xls_parser(report_base_vn.Parser):
    ...
    def get_wizard_data(self):
        ...
        if datas:
            ...
            result['foreign_currency_id'] = datas['form'] and datas['form'][
                'foreign_currency_id'] and datas['form']['foreign_currency_id'][0] or False
            result['filter'] = 'filter' in datas['form'] and datas['form']['filter'] or False
            result['partner_id'] = datas['form'] and datas['form']['partner_id'] and datas['form']['partner_id'][0] or False
            result['account_type'] = datas['form'] and datas['form']['result_selection']
    ...
    def get_lines_data(self):
        ...
        previous_date_from_date = datetime.strptime(
            date_info['date_from_date'], DEFAULT_SERVER_DATE_FORMAT) - timedelta(days=1)
        params = {'target_move': '',
        ...
        SQL = """
                ...
                ORDER BY partner_id desc, effective_date -- very important to order by DESC
...
class account_foreign_receivable_payable_ledger_xls(report_xlsx):
    ...
    def generate_xls_report(self, _p, _xs, data, objects, workbook):
        ...
        self.style_date = workbook.add_format(
            {'font_name': 'Arial', 'font_size': 10, 'num_format': 'dd/mm/yy', 'text_wrap': True})
        self.style_date_right = workbook.add_format(
            {'font_name': 'Arial', 'font_size': 10, 'num_format': 'dd/mm/yy', 'align': 'right', 'text_wrap': True})
        self.style_date_right_border = workbook.add_format(
            {'font_name': 'Arial', 'font_size': 10, 'num_format': 'dd/mm/yy', 'align': 'right', 'text_wrap': True, 'border': 1})
        self.style_date_center_border = workbook.add_format(
            {'font_name': 'Arial', 'font_size': 10, 'num_format': 'dd/mm/yy', 'align': 'center', 'text_wrap': True, 'border': 1})
        ...
        self.style_decimal_bold_border = workbook.add_format(
            {'font_name': 'Arial', 'font_size': 10, 'num_format': '#,##0', 'bold': True, 'text_wrap': True, 'border': 1})
        ...
        self.normal_style_left_borderall = workbook.add_format(
            {'font_name': 'Arial', 'font_size': 10, 'align': 'left', 'valign': 'vjustify', 'border': 1, 'text_wrap': True})
        self.normal_style_right_borderall = workbook.add_format(
            {'font_name': 'Arial', 'font_size': 10, 'align': 'right', 'valign': 'vjustify', 'border': 1, 'text_wrap': True})
        self.normal_style_italic = workbook.add_format(
            {'font_name': 'Arial', 'font_size': 10, 'align': 'center', 'valign': 'vjustify', 'italic': True, 'text_wrap': True})

        self.normal_style = workbook.add_format(
            {'font_name': 'Arial', 'font_size': 10, 'align': 'left', 'text_wrap': True})
        self.normal_style_borderall = workbook.add_format(
            {'font_name': 'Arial', 'font_size': 10, 'align': 'center', 'valign': 'vcenter', 'border': 1, 'text_wrap': True})
        self.normal_style_bold = workbook.add_format(
            {'font_name': 'Arial', 'font_size': 10, 'align': 'center', 'valign': 'vjustify', 'bold': True, 'text_wrap': True})
        self.normal_style_bold_left_borderall = workbook.add_format(
            {'font_name': 'Arial', 'font_size': 10, 'align': 'left', 'valign': 'vjustify', 'bold': True, 'text_wrap': True, 'border': 1})
        self.normal_style_bold_borderall = workbook.add_format(
            {'font_name': 'Arial', 'font_size': 10, 'align': 'center', 'valign': 'vcenter', 'bold': True, 'border': 1, 'text_wrap': True})
        ...
        report_name = u'SỔ CHI TIẾT THANH TOÁN VỚI NGƯỜI %s BẰNG NGOẠI TỆ' % NAME
        ...
        cell_address_style = workbook.add_format(
            {'font_name': 'Arial', 'font_size': 10, 'align': 'left', 'bold': True, 'text_wrap': True})
        # Title address 1
        c_specs = [
        ...
        c_specs = [
            ...
            ('layout', 8, 0, 'text', u'(Ban hành theo QĐ số 48/2006/QĐ-BTC, Ngày 14/09/2006 của Bộ trưởng BTC)',
             '', self.normal_style_center),
        ]
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        row_pos = self.xls_write_row(
        ...
        cell_title_style = workbook.add_format(
            {'font_name': 'Arial', 'align': 'center', 'valign': 'vcenter', 'bold': True, 'text_wrap': True, 'font_size': 18})
        ws.set_row(row_pos, 22)
        c_specs = [
            ('payment_journal', 14, 0, 'text', report_name, None, cell_title_style)
        ]
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        row_pos = self.xls_write_row(
            ws, row_pos, row_data, row_style=self.normal_style)
        ...
        # Loại ngoại tệ: USD
        currency_name = data['form'] and data['form']['foreign_currency_id'] and data['form']['foreign_currency_id'][1] or ''
        c_specs = [
        ...
        ws.merge_range(row_title_body_pos, 0, row_title_body_pos + 1,
                       0, u'Ngày tháng ghi sổ', self.normal_style_bold_borderall)
        ...
        # @UnusedVariable
        for partner_id, group_datas in groupby(get_lines_data, lambda line: line['partner_id']):
            #
            ...
            if group_datas[0].get('description') == 'SDDK':
                ...
                if foreign_overbalance_amount > 0:
                    begining_c_specs = self.gen_sddk_c_cpecs(
                        debit_begin, credit_begin, foreign_overbalance_amount, 0)
                else:
                    begining_c_specs = self.gen_sddk_c_cpecs(
                        debit_begin, credit_begin, 0, foreign_overbalance_amount)
                row_pos = self.write_sddk(
                    ws, row_pos, begining_c_specs, row_style=self.normal_style_borderall)
                group_datas = group_datas[1:]

                #####

                # Sum total and ending
                end_foreign_debit += foreign_overbalance_amount > 0 and foreign_overbalance_amount or 0
                end_foreign_credit += foreign_overbalance_amount < 0 and foreign_overbalance_amount or 0
            ...
            for line in group_datas:
                row_pos, overbalance_amount, foreign_overbalance_amount, end_debit, end_credit, sum_foreign_debit,\
                    sum_foreign_credit, sum_debit, sum_credit, end_foreign_debit,\
                    end_foreign_credit = self.write_lines(ws, row_pos, line, overbalance_amount, foreign_overbalance_amount,
            ...
            cps_c_specs, ending_c_specs = self.gen_cps_ending_c_cpecs(sum_foreign_debit, sum_debit,
            ...
            row_pos = self.xls_write_row(
                ws, row_pos, row_data, row_style=self.normal_style_borderall, set_column_size=True)
        ...
        ###############
        cell_footer_style = workbook.add_format(
            {'font_name': 'Arial', 'font_size': 10, 'align': 'center', 'italic': True})
        empty = [('empty%s' % x, 1, 0, 'text', '') for x in range(11)]
        c_specs = [
            ('col1', 1, 0, 'text', '', None),
            ('col2', 1, 0, 'text', '', None),
    ...
    def write_lines(self, ws, row_pos, line, overbalance_amount, foreign_overbalance_amount, end_debit, end_credit,
                    sum_foreign_debit, sum_foreign_credit, sum_debit, sum_credit, end_foreign_debit, end_foreign_credit, row_style):
        #  Write lines
        debit = line['debit']
        credit = line['credit']
        ...
        overbalance_amount, overbalance_debit, overbalance_credit, end_debit, end_credit =\
            self.calculate_overbalance(overbalance_amount,
                                       debit,
                                       credit)

        foreign_overbalance_amount, overbalance_foreign_debit, overbalance_foreign_credit, end_foreign_debit, end_foreign_credit =\
            self.calculate_overbalance(foreign_overbalance_amount,
                                       foreign_debit,
                                       foreign_credit)
    ...
    def gen_sddk_c_cpecs(self, debit_begin, credit_begin, foreign_debit_begin, foreign_credit_begin):
        begining_c_specs = [
            ('col1', 1, 12, 'text', '', None),
            ('col2', 1, 12, 'text', '', None),
            ('col3', 1, 12, 'text', '', None),
    ...
    def gen_cps_ending_c_cpecs(self, sum_foreign_debit, sum_debit, sum_foreign_credit, sum_credit,
...
account_foreign_receivable_payable_ledger_xls(
    'report.foreign_receivable_payable_ledger_report', 'account.move.line', parser=account_foreign_receivable_payable_ledger_xls_parser)
