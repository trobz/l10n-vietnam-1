...
#
#    Copyright (c) 2014 Trobz nv/sa (trobz.com). All rights reserved.
#
...
class asset_book_xls_parser(report_base_vn.Parser):
    ...
    def __init__(self, cr, uid, name, context):
        super(asset_book_xls_parser, self).__init__(cr, uid, name, context=context)
    ...
    def get_wizard_data(self):
        ...
        if datas:
            ...
            result['chart_account_id'] = datas['form'] and datas['form']['chart_account_id'] or False
            result['asset_category_id'] = datas['form'] and datas['form']['asset_category_id'] and [datas['form']['asset_category_id'][0]] or \
            self.pool.get('account.asset.category').search(self.cr, self.uid, [])
    ...
    def get_asset_data(self):
        ...
        params  = { 'category_ids': tuple(category_ids + [-1,-1]), 'date_start': date_info['date_from_date'], 'date_end': date_info['date_to_date'] }
        ...
        SQL = """
            ...
            COALESCE(country.name,'') as manufacturer,manufacture_year,department,start_date_in_use,
            capacity,date_of_liquidation,reason,
            aml.date_created AS effective_date,move_check,
            purchase_value, adl.remaining_value, (asset.purchase_value - adl.remaining_value) as amount_depreciated,
            ...
            CASE WHEN method = 'linear'
                THEN (SELECT amount FROM account_asset_depreciation_line WHERE asset.id=asset_id LIMIT 1)
                ELSE 0
            END AS current_deprecition

            FROM account_asset_asset asset
            LEFT JOIN account_invoice as ai ON ai.number=asset.code
            LEFT JOIN (SELECT min(date_created) as date_created,asset_id FROM account_move_line GROUP BY asset_id) AS aml
                ON aml.asset_id=asset.id
            LEFT JOIN (SELECT MIN(remaining_value) as remaining_value ,asset_id,move_check
    ...
    def get_report_param(self, param=''):
        report_param = self.pool.get('ir.config_parameter').get_param(self.cr, self.uid, 'account_asset_book_report_parameter')
...
class asset_book_xls(report_xls,report_base_vn.Parser):
    ...
    def __init__(self, name, table, rml=False, parser=False, header=True, store=False):
        super(asset_book_xls, self).__init__(name, table, rml, parser, header, store)
        ...
        _xs = self.xls_styles
        # header
        rh_cell_format = _xs['bold'] + _xs['fill'] + _xs['borders_all'] + _xs['wrap'] + _xs['middle']
        ...
        self.aml_cell_style_date = xlwt.easyxf(aml_cell_format + _xs['left'] + _xs['center'], num_format_str=date_format )
        self.aml_cell_style_decimal = xlwt.easyxf(aml_cell_format + _xs['right'], num_format_str=decimal_format)
        # totals
        rt_cell_format = _xs['bold'] + _xs['fill'] + _xs['borders_all'] + _xs['wrap'] + _xs['middle']
        self.rt_cell_style = xlwt.easyxf(rt_cell_format)
        self.rt_cell_style_right = xlwt.easyxf(rt_cell_format + _xs['right'])
        self.rt_cell_style_decimal = xlwt.easyxf(rt_cell_format + _xs['right'], num_format_str=decimal_format)
        ...
        # normal
        self.cell_style_normal = xlwt.easyxf(self.asset_xls_styles['normal'] + _xs['borders_all'] + _xs['wrap'] + _xs['center'] +  _xs['middle'])
        self.cell_style_normal_borderless = xlwt.easyxf(self.asset_xls_styles['normal'] + _xs['wrap'] + _xs['center'] +  _xs['middle'])
        self.cell_style_normal_borderless_italic = xlwt.easyxf(self.asset_xls_styles['normal'] + _xs['wrap'] + _xs['center'] +  _xs['middle'] + _xs['italic'])
        cell_total_style = xlwt.easyxf(_xs['wrap'] + _xs['center'] + _xs['bold']  + _xs['borders_all'])

        # center
        self.cell_style_center = xlwt.easyxf(_xs['center'] + _xs['borders_all'] + _xs['wrap']+ _xs['middle'])

        # XLS Template
        self.wanted_list = ['A','B','C','D','E','G','H','1','2','3','4','I','K','L']
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        # Title 1
        c_specs = [
            ('company_name', 3, 0, 'text', u'Đơn vị: %s' % _p.get_company()['name']  ,'', cell_address_style),
            ('empty1', 1, 0, 'text', ''),
            ('empty2', 1, 0, 'text', ''),
            ('empty3', 1, 0, 'text', ''),
            ('empty4', 1, 0, 'text', ''),
            ...
            ('empty6', 1, 0, 'text', ''),
            ('empty7', 1, 0, 'text', ''),
            ('form_serial', 4, 0, 'text', _p.get_report_param('format_form') , '', cell_footer_style)
        ]
        ...
        c_specs = [
            ('company_name', 3, 0, 'text', u'Địa chỉ: %s' % _p.get_company()['address'], '', cell_address_style),
            ('empty1', 1, 0, 'text', ''),
            ('empty2', 1, 0, 'text', ''),
            ('empty3', 1, 0, 'text', ''),
            ('empty4', 1, 0, 'text', ''),
            ...
            ('empty6', 1, 0, 'text', ''),
            ('empty7', 1, 0, 'text', ''),
            ('form_serial', 4, 0, 'text', _p.get_report_param('form_formated_by_rule'), '', self.cell_style_normal_borderless)
        ]
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        row_pos = self.xls_write_row(ws, row_pos, row_data, row_style=self.cell_style_normal_borderless)
        ...
        ws.row(row_pos).height_mismatch = True
        ws.row(row_pos).height = 256*2
        cell_format = _xs['bold'] + _xs['wrap'] + _xs['center'] + 'font: height 370;'
        cell_title_style = xlwt.easyxf(cell_format)

        empty = [('empty%s' % x, 1, 0, 'text', '') for x in range(5)]
        c_specs = empty + [
            ('asset_book', 4, 10, 'text','SỔ TÀI SẢN CỐ ĐỊNH', None, cell_title_style)
        ]
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        row_pos = self.xls_write_row(ws, row_pos, row_data, row_style=self.cell_style_normal_borderless)
        ...
        c_specs = empty + [
            ('asset_book', 4, 0, 'text','Từ %s Đến %s' % (_p.get_date().get('date_from','.......'),_p.get_date().get('date_to','.......')))
        ]
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        row_pos = self.xls_write_row(ws, row_pos, row_data, row_style=self.cell_style_normal_borderless_italic)
        ...
        cell_title_style = xlwt.easyxf(cell_format)
        c_specs = empty + [
            ('from_to', 4, 10, 'text', u'Loại tài sản: %s' % _p.get_asset_category(), None)
        ]
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        ...
        c_specs = [
            ...
            ('increase_asset', 7, 0, 'text', 'Ghi tăng TSCĐ', None, self.cell_style_normal),
            ('depreciation_asset', 3, 0, 'text', 'Khấu hao TSCĐ', None, self.cell_style_normal),
            ('decrease_asset', 3, 0, 'text', 'Ghi giảm TSCĐ', None, self.cell_style_normal)
        ]
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        row_pos = self.xls_write_row(ws, row_pos, row_data, row_style=self.cell_style_normal)
        ...
        c_specs = [
            ...
            ('col3', 1, 0, 'text', 'Tên, đặc điểm, ký hiệu TSCĐ', None, self.cell_style_center),
            ('col4', 1, 0, 'text', 'Nước sản xuất', None, self.cell_style_center),
            ('col5', 1, 0, 'text', 'Tháng năm đưa vào sử dụng', None, self.cell_style_center),
            ('col6', 1, 0, 'text', 'Số hiệu TSCĐ', None, self.cell_style_center),
            ('col7', 1, 0, 'text', 'Nguyên giá TSCĐ', None, self.cell_style_center),
            ('col8', 2, 0, 'text', 'Khấu hao', None, self.cell_style_center),
            ('col9', 1, 0, 'text', 'Khấu hao đã tính đến khi giảm TSCĐ', None, self.cell_style_center),
            ('col10', 2, 0, 'text', 'Chứng từ', None, self.cell_style_center),
            ('col11', 1, 0, 'text', 'Lý do giảm TSCĐ', None, self.cell_style_center)
        ]
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        row_pos = self.xls_write_row(ws, row_pos, row_data, row_style=self.cell_style_normal)
        ...
        c_specs = [
            ...
            ('col9', 1, 0, 'text', 'Tỷ lệ (%) khấu hao', None, self.cell_style_center),
            ('col10', 1, 0, 'text', 'Mức khấu hao', None, self.cell_style_center),
            ('col11', 1, 0, 'text', '', None, self.cell_style_center),
            ('col12', 1, 0, 'text', 'Số hiệu', None, self.cell_style_center),
            ('col13', 1, 0, 'text', 'Ngày, tháng, năm', None, self.cell_style_center),
        ...
        # merge cell
        ws.write_merge(row_title_body_pos - 1, row_title_body_pos+1, 0, 0, 'STT', self.aml_cell_style_center )
        ws.write_merge(row_title_body_pos, row_title_body_pos+1, 3, 3, 'Tên, đặc điểm, ký hiệu TSCĐ', self.aml_cell_style_center )
        ws.write_merge(row_title_body_pos, row_title_body_pos+1, 4, 4, 'Nước sản xuất', self.aml_cell_style_center )
        ws.write_merge(row_title_body_pos, row_title_body_pos+1, 5, 5, 'Tháng năm đưa vào sử dụng', self.aml_cell_style_center )
        ws.write_merge(row_title_body_pos, row_title_body_pos+1, 6, 6, 'Số hiệu TSCĐ', self.aml_cell_style_center )
        ws.write_merge(row_title_body_pos, row_title_body_pos+1, 7, 7, 'Nguyên giá TSCĐ', self.aml_cell_style_center )
        ws.write_merge(row_title_body_pos, row_title_body_pos+1, 10, 10, 'Khấu hao đã tính đến khi giảm TSCĐ', self.aml_cell_style_center )
        ws.write_merge(row_title_body_pos, row_title_body_pos+1, 13, 13, 'Lý do giảm TSCĐ', self.aml_cell_style_center )

        ws.row(row_title_body_pos+1).height_mismatch = True
        ws.row(row_title_body_pos+1).height = 256*2
        # Column headers
        c_specs = map(lambda x: self.render(x, self.col_specs_template, 'header'), self.wanted_list)
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        row_pos = self.xls_write_row(ws, row_pos, row_data, row_style=self.aml_cell_style_center, set_column_size=True)
        ...
        for line in asset_data: # @UnusedVariable
            ...
            row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
            row_pos = self.xls_write_row(ws, row_pos, row_data, row_style=self.aml_cell_style_center)
        ...
        sum_purchase_value_formula = 'SUM(%s:%s)' % (purchase_value_start, purchase_value_stop) # @UnusedVariable
        c_specs = map(lambda x: self.render(x, self.col_specs_template, 'totals'), self.wanted_list)
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        row_pos = self.xls_write_row(ws, row_pos, row_data, row_style=self.aml_cell_style_decimal)
        ...
        c_specs = [
            ('note1', 5, 0, 'text','- Sổ này có ........ trang, đánh từ trang 01 dến trang ........', None)
        ]
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
