...
class account_purchases_journal_xls_parser(report_base_vn.Parser):
    ...
    def __init__(self, cr, uid, name, context):
        super(account_purchases_journal_xls_parser, self).__init__(cr, uid, name, context=context)
    ...
    def get_lines_data(self):
        ...
        params  = { 'target_move': '', 'date_start': date_info['date_from_date'], 'date_end': date_info['date_to_date'] }
        ...
        */
        ...
        FROM
            ...
            JOIN account_account dr_account ON move_dr.account_id = dr_account.id
            JOIN account_account cr_account ON move_cr.account_id = cr_account.id
        WHERE
            move_cr.credit != 0
            AND move_dr.debit != 0
...
class account_purchases_journal_xls(report_xls_utils.generic_report_xls_base):
    ...
    def __init__(self, name, table, rml=False, parser=False, header=True, store=False):
        super(account_purchases_journal_xls, self).__init__(name, table, rml, parser, header, store)
    ...
    def generate_xls_header(self, _p, _xs, data, objects, wb, ws, row_pos, report_name):
        ...
        c_specs = [
            ('from_to', 7, 0, 'text','Từ %s đến %s' % (_p.get_date().get('date_from','.......'),_p.get_date().get('date_to','.......')))
        ]
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        ...
        ws.write_merge(row_title_body_pos, row_title_body_pos+1, 4, 4, 'Tài khoản ghi nợ', self.normal_style_bold_borderall )
        ws.write_merge(row_title_body_pos, row_title_body_pos+1, 5, 5, 'Tài khoản ghi có', self.normal_style_bold_borderall )
        ws.write_merge(row_title_body_pos, row_title_body_pos+1, 6, 6, 'Số tiền', self.normal_style_bold_borderall )
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        # set print sheet
        ws = super(account_purchases_journal_xls, self).generate_xls_report(_p, _xs, data, objects, wb, report_name)
        row_pos = self.generate_xls_header(_p, _xs, data, objects, wb, ws, 0, report_name)
        MAX_ROW = 65500
        count = 1
...
#         # account move lines
        ...
        if not get_lines_data:
            ...
            self.col_specs_template['H']['totals'] = self.col_specs_template['H']['totals_no_lines']
...
account_purchases_journal_xls('report.purchases_journal_report','account.move', parser=account_purchases_journal_xls_parser)
