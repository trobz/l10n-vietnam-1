...
class Parser(report_base_vn.Parser):
    ...
    def get_result_base_on_target(self):
        ...
        for invoice in invoice_obj.browse(self.cr, self.uid, invoice_ids):
            ...
            item = {
                ...
                'invoice_num': invoice.type == 'out_invoice' and invoice.customer_invoice_num or invoice.supplier_invoice_number,
            ...
            for line in invoice.invoice_line:
                ...
                if not is_vnd:
                    sub_total = currency_obj.compute(self.cr, self.uid,
                                                     invoice.currency_id.id,
                                                     invoice.company_id.currency_id.id,
                                                     line.price_subtotal,
                                                     context={
                                                         'date': invoice.date_invoice})
                    sub_total_tax = currency_obj.compute(self.cr, self.uid,
                                                         invoice.currency_id.id,
                                                         invoice.company_id.currency_id.id,
                                                         sub_total_tax,
                                                         context={
    ...
    def get_wizard_data(self):
        data = self.localcontext['data']['form']
        date_start_str = self.pool.get('account.period').read(self.cr,
                                                              self.uid, data[
