...
SELECT row_to_json(final) FROM (
    SELECT
        ...
        , coalesce(sum(CASE WHEN acc.group_by_partner ISNULL OR acc.group_by_partner = 'f'
        THEN acc_sum.begin_period ELSE 0 END), 0)
         AS begin_period_by_acc

        , coalesce(sum(CASE WHEN acc.group_by_partner = 't' AND acc_sum.begin_period > 0
        ...
        -- , coalesce(sum(CASE WHEN acc.group_by_partner = 't' AND acc_sum.in_period > 0
        -- THEN acc_sum.in_period ELSE 0 END), 0) AS dr_in_period

        -- , coalesce(sum(CASE WHEN acc.group_by_partner = 't' AND acc_sum.in_period < 0
        -- THEN -acc_sum.in_period ELSE 0 END), 0) AS cr_in_period

        , coalesce(sum(CASE WHEN acc.group_by_partner = 't' AND acc_sum.end_period > 0
        ...
        while max_lev > 0:
            ...
            for k_lead, v_lead in lead.items():
                to_update = {
                    ...
                    if set(v['path']) < set(v_lead['path'])
                    and max_lev == v['level'] + 1
                }
                for k in to_update:
